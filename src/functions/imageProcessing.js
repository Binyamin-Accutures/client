...
async function convertRGB(
  imgUrl,
  { minDoLPVal, maxDoLPVal, minS0Value, maxS0Value, AoLPCenter, AoLPPov }
) {
  ...
  const camanjsPromise = new Promise((resolve, reject) => {
    window.Caman(canvas, img, function () {
      this.process("posterize", function (rgba) {
        rgba.r = Math.floor(
          clip((rgba.r - (AoLPCenter - AoLPPov / 2)) / AoLPPov, 0, 255)
        );
        rgba.g = Math.floor(
          clip((rgba.g - minDoLPVal) / (maxDoLPVal - minDoLPVal), 0, 255)
        );
        rgba.b = Math.floor(
          clip((rgba.b - minS0Value) / (maxS0Value - minS0Value), 0, 255)
        );
        return rgba;
      }).render(() => {
        resolve();
...
async function convertAoLPOvealayed(imgUrl, processParamters) {
  const { HSFactor, minS0Value, maxS0Value } = processParamters;
  ...
  const camanjsPromise = new Promise((resolve, reject) => {
    window.Caman(canvas, img, function () {
      ...
      this.process("posterize", function (rgba) {
        ...
        const H = clip(
          (HSV.h - MIN_VALUE_HSV) / (HSFactor - MIN_VALUE_HSV),
          0,
          1
        );
        const S = clip(
          (HSV.s - MIN_VALUE_HSV) / (DoLPSatur - MIN_VALUE_HSV),
          0,
          1
        );
        const V = clip(
          (HSV.v - MIN_VALUE_HSV) / (MAX_VALUE_HSV - MIN_VALUE_HSV),
          0,
          1
        );
  ...
  const camanjsPromise = new Promise((resolve, reject) => {
    window.Caman(canvas, img, function () {
      this.process("posterize", function (rgba) {
        ...
        const S = clip(
          (HSV.s - minSaturation) / (maxSaturation - minSaturation),
          0,
          1
        );
